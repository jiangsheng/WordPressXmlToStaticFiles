...
namespace WordPressXmlToJekykll
{
    class WordPressXml
    {
        ...
        public static WordPressXml FromFile(string inputFile)
        {
            ...
            if (channel != null)
            {
                result.Author = WordPressAuthor.FromXmlNode(channel.SelectSingleNode("wp:author", nsmgr), nsmgr);
                result.Categories=WordPressCategory.FromXmlNode(channel.SelectNodes("wp:category", nsmgr), nsmgr);
                result.Tags=WordPressTag.FromXmlNode(channel.SelectNodes("wp:tag", nsmgr), nsmgr));
                result.Items = WordPressItem.FromXmlNode(channel.SelectNodes("item", nsmgr), nsmgr);
            }
            return result;
        }
